...
function parseRow(headerToIdx, row) {
  ...
  return {
    ...
    capacity: {
      ...
      // In some exports this header may include a trailing space; headers are trimmed in buildPayload.
...
function buildPayload() {
  ...
  // Fail fast if a required column was renamed.
  const required = [
    "Building Ref ID (Text)",
    "Spaces Location ID",
    "Site Name",
    "Location",
    "State",
  ];
  const missing = required.filter((h) => !(h in headerToIdx));
  if (missing.length) {
    throw new Error(`reference.xlsx is missing required column(s): ${missing.join(", ")}`);
  }
  ...
  // Canonical sort by siteName (idempotency)
...
function renderBlock(payload) {
  ...
  const lines = [
    ...
    "(function(){",
    `var SITES = ${sitesJson};`,
    `var BY_SITE_NAME = ${JSON.stringify(payload.idxBySiteName, null, 2)};`,
    `var BY_BUILDING_REF = ${JSON.stringify(payload.idxByBuildingRefId, null, 2)};`,
    `var BY_LOC_UPPER = ${JSON.stringify(payload.idxByLocationUpper, null, 2)};`,
    `var BY_SPACES_LOC = ${JSON.stringify(payload.idxBySpacesLocationId, null, 2)};`,
